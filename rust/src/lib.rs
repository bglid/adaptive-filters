...
mod sample_buffer;
use sample_buffer::{SampleBuffer, SampleView};

mod errors;
use errors::{FilterError, FilterResult};
...
mod tests {
    ...
    impl Algorithm for TestAlgorithm {
            ...
            T: SampleView,
        {
            for (i, w) in weights.iter_mut().enumerate() {
                *w += e_n[0] * x_n.get(i).unwrap();
            }
        }
    }

    fn testing_filter() -> Filter<TestAlgorithm> {
        let mut filter = Filter::<TestAlgorithm>::new(TestAlgorithm {}, 3).unwrap();
    ...
    fn error() {
        assert!(approx_equal(
            Filter::<TestAlgorithm>::error(5.0, 3.5),
            1.5,
            1e-6
        ));
    }
